...
# BUG: champs for which you currently have 0 stones are not shown in the results table
...
# %%
def main(
    directory: str = "C:\\Users\\Felix\\Desktop\\auf-zu-win-11\\th\\2023SS\\oddities\\ClairStone\\Backend\\cards_data\\",
    gui: bool = False,
):
    try:
        # console setup
        os.system(f"mode con cols={1000} lines={1000}")
        ...
        # one backslash is purely an escape character
        ...
        user_items_raw = pd.read_json(directory + "user_items.json", orient="index")
        # print(user_items_raw)

        item_data_raw = pd.read_json(directory + "item_data.json", orient="index")
        # print(item_data_raw.head())

        user_units_raw = pd.read_json(directory + "user_units.json", orient="index")
        # print(user_units_raw.head())

        # use etree for cards_shard.xml
        cards_shard = etree.parse(directory + "cards_shard.xml")  # type: ignore
        ...
        input(
            "Press ENTER to try fetching new data... (Else restart the flask_extractor.exe, check your files manually, check README.txt, then message IchChefDuNixx)"
        )
        return

    try:
        # constants for id boundaries (10.06.23), easier to change here than in-line
        flasks_id_min = 2534
        flasks_id_max = 2681

        # reward, premium aether, chaos, wyld
        champion_id_min = [4000, 5600, 6600, 7600]
        champion_id_max = [4999, 5999, 6999, 7999]

        # shard ids are auto-generated in-game as well
        champion_shards_id_min = [id_min + 100000 for id_min in champion_id_min]
        champion_shards_id_max = [id_max + 100000 for id_max in champion_id_max]
        ...
        item_data_flask_ids = item_data_raw[
            item_data_raw["name"].str.contains("Flask")
        ][
            4:
        ...
        if flasks_id_min_temp <= flasks_id_min and flasks_id_max_temp >= flasks_id_max:
            ...
            print(
                "Error in #1: this action could possibly exclude flasks. Message IchChefDuNixx"
            )
        ...
        item_data_flask_rewards = pd.DataFrame(
            rows,
            columns=["id", "flask_name", "reward1", "reward2", "reward3", "reward4"],
        )

        # 3 create new dataframe with flask id and amount owned
        user_items_flasks = user_items_raw[
            user_items_raw["id"].isin(item_data_flask_rewards["id"])
        ].merge(item_data_flask_rewards)

        # 5 create new dataframe with total amount of shards available per champion (very sql like)
        user_items_flasks_melted = user_items_flasks.melt(
            id_vars=["id", "number"],
            value_vars=["reward1", "reward2", "reward3", "reward4"],
            var_name="reward_col",
            value_name="reward",
        )

        user_items_flasks_grouped = (
            user_items_flasks_melted.groupby(["reward"])
        ...
        item_data_shard_source = pd.DataFrame.from_dict(
            source_dict,
            orient="index",
            columns=[
                f"source_{i}"
                for i in range(1, max([len(val) for val in source_dict.values()]) + 1)
            ],
        )

        # merge previous list with shard sums with shard source (EXLUDING YET UNBUILT CHAMPIONS)
        item_data_shards = user_items_flasks_grouped.merge(
            item_data_shard_source, left_on="reward", right_index=True
        )
        item_data_shards.rename(
            columns={"id": "num_flasks", "number": "flask_shards"}, inplace=True
        )
        ...
        # first set all owned shard values to zero, then overwrite with true values if non-zero. literally don't overwrite zeros
        owned_champ_ids = user_units_raw[
            user_units_raw["unit_id"].apply(
                lambda x: any(
                    a <= x <= b for a, b in list(zip(champion_id_min, champion_id_max))
                )
            )
        ]["unit_id"].values

        owned_champ_ids = [
            owned_champ_id + 100000 for owned_champ_id in owned_champ_ids
        ]

        user_items_champion_shards = pd.DataFrame(
            {"id": owned_champ_ids, "number": [0] * len(owned_champ_ids)}
        ).set_index("id", drop=False)

        for id_min, id_max in zip(champion_shards_id_min, champion_shards_id_max):
            user_items_champion_shards.update(
                user_items_raw[user_items_raw["id"].between(id_min, id_max)]
            )

        # TODO: use .update() like above to shorten code
        ...
        for id_min, id_max in zip(champion_id_min, champion_id_max):
            user_units_champions_owned.append(
                user_units_raw[user_units_raw["unit_id"].between(id_min, id_max)]
            )
        ...
        user_units_champions_owned = user_units_champions_owned.drop(
            user_units_champions_owned.columns.difference(["unit_id", "level"]), axis=1
        )

        # FIX: add unbuilt champs and stone amounts
        # e.g. [105600, 105604, 105610, 105615, 106603, 106613, 107601, 107612]
        level_zero_champ_ids = list(
            set(user_items_raw["id"][user_items_raw["id"] >= 100000].values)
            - set(owned_champ_ids)
        )

        for level_zero_champ in level_zero_champ_ids:
            # champ
            user_units_champions_owned.loc[
                user_units_champions_owned.index.max() + 1
            ] = [
                level_zero_champ - 100000,
                0,
            ]
            # stones
            user_items_champion_shards.loc[level_zero_champ] = [
                level_zero_champ,
                user_items_raw.loc[level_zero_champ]["number"],
            ]
        ...
        # convert from shards_id to unit_id
        ...
        full_frame = full_frame.merge(
            item_data_shards, how="left", left_on="id", right_on="reward"
        )
        ...
        full_frame["is_sufficient"] = full_frame.apply(
            lambda x: is_sufficient(x["number"], x["flask_shards"], x["upgrade_cost"]),
            axis=1,
        )

        full_frame.tail()

        user_items_flasks.set_index(
            "id", inplace=True
        )  # only runnable once before issues arise
            ...
            for row in col:
                if not pd.isna(row):
                    if row not in replace_dict:
                        replace_dict[row] = (
                            user_items_flasks.loc[row, "flask_name"],
                            user_items_flasks.loc[row, "number"],
                        )  # shardbot exception perma fix here
        ...
        if not gui:
            ...
            return (
                final  # only if everything before worked correctly, without exceptions
            )
    except Exception as e:
        print(
            "Exception while processing data occured!\nCannot continue!\nCheck README.txt, then message IchChefDuNixx"
        )
        print(e)
        print(e.args)
...
def not_main(gui: bool = False):
    ...
    try:
        response = requests.get(
            f"https://spellstone.synapse-games.com/api.php?message=init&user_id={credentials['user_id']}&password={credentials['password']}"
        )
        ...
        # champ xml update
        response = requests.get(
            "https://spellstone.synapse-games.com/assets/cards_shard.xml"
        )
